...
{
    public class Earth : TexturedBall
    {
        public string NightFilename;
        public int NightTexture;

        public override void LoadTexture()
        {
            base.LoadTexture();
            if (NightFilename == null)
                return;
            GL.GenTextures(1, out NightTexture);
            GL.BindTexture(TextureTarget.Texture2D, NightTexture);
            var bitmap = new Bitmap(NightFilename);
            Console.WriteLine(@"Binding {0} to {1}", NightFilename, NightTexture);
        ...
        public override void Paint()
        {
            ...
            {
                //int u;
                //GL.GetInteger(GetPName.ActiveTexture, out u);
                //Console.WriteLine(@"Active texture before change={0}", u);
                GL.ActiveTexture(TextureUnit.Texture1);
                //GL.GetInteger(GetPName.ActiveTexture, out u);
                //Console.WriteLine(@"Active texture  after change={0}", u);
                GL.BindTexture(TextureTarget.Texture2D, NightTexture); // NightTexture
            }
