...
namespace Shadow.spice
{
    public class CSpice
    {
        ...
        public static double[] XAxis = new[] {1d, 0d, 0d};
        public static double[] YAxis = new[] {0d, 1d, 0d};
        public static double[] ZAxis = new[] {0d, 0d, 1d};
        ...
        {
            var filenames = kernels.Split(',').Select(name => name.Trim()).ToArray();
        ...
        public void LoadStandardKernels(string[] filenames)
        {
            ...
            {
                var fullname = Path.GetFullPath(Path.Combine(SpiceKernelDirectory, filename));
        ...
        public void GetLadeeLonLatAlt(long time, out double longitude, out double lattitude, out double altitude)
        {
            var et = TimeUtilities.Time42ToET(time);
            var ladeePos = new double[3];
            var lt = 0d;
            spkezp_c(LadeeId, et, "MOON_ME", "NONE", MoonId, ladeePos, ref lt);
            double ladeeLon = 0d, ladeeLat = 0d, ladeeAlt = 0d;
            recpgr_c("MOON", ladeePos, MoonRadius, 0d, ref ladeeLon, ref ladeeLat, ref ladeeAlt);
            longitude = ladeeLon*180d/3.141592653589d;
            lattitude = ladeeLat*180d/3.141592653589d;
            altitude = ladeeAlt;
        }
        ...
        public void GetLadeeSolarLonLatAlt(long time42, out double solarLongitude, out double solarLattitude,
                                           out double altitude)
        {
            // Convert epoch to ephemeris time
            var et = TimeUtilities.Time42ToET(time42);
            var lt = 0d;

            var xform = GetMoonFixedToMoonSolarTransform(et);
            // get spacecraft position
            var scPos2 = new double[3];
            spkezp_c(LadeeId, et, "MOON_ME", "NONE", MoonId, scPos2, ref lt);
            // transform position to local time
            var scPos = new double[3];
            mxv_c(xform, scPos2, scPos);

            // I don't understand the bug, but the solar lattitude is negative
            scPos[2] = -scPos[2];

            // convert to lon, and lat
            solarLongitude = solarLattitude = altitude = 0d;
            recpgr_c("MOON", scPos, MoonRadius, MoonFlattening, ref solarLongitude, ref solarLattitude, ref altitude);
            solarLattitude = ToDegrees(solarLattitude);
            solarLongitude = ToDegrees(solarLongitude);
        }

        public static double ToDegrees(double rad)
        {
            return rad*180d/3.14159265358979323846626d;
        }

        public double[] GetSubsolarPointLonLat(long time42)
        {
            // Convert epoch to ephemeris time
            var et = TimeUtilities.Time42ToET(time42);
            ...
            // Get target position
            var lt = 0d;
            var surfacePt = new double[3];
            var surfaceVec = new double[3];

            subpnt_c("Intercept: ellipsoid", "LADEE", et, "MOON_ME", "NONE", "SUN", surfacePt, ref lt, surfaceVec);

            // convert to radius, lon, and lat
            double lon = 0d, lat = 0d, alt = 0d;
            recpgr_c("MOON", surfacePt, MoonRadius, MoonFlattening, ref lon, ref lat, ref alt);
            lon = ToDegrees(lon);
            lat = ToDegrees(lat);
            return new[] {lon, lat, alt};
        }

        public void GetEastGrazingPoint(long time42, out double grazeLongitude, out double grazeLattitude, out double grazeAltitude,
                                        out double solarLongitude, out double solarLattitude)
        {
            ...
            spkgeo_c(-12, et, "MOON_ME", 301, ladeeState, ref lt);

            var grazeVector = new double[3];
            grazeVector[0] = -ladeeState[3];
            grazeVector[1] = -ladeeState[4];
            grazeVector[2] = -ladeeState[5];

            GetGrazingPointInner(time42, ladeeState, grazeVector, out grazeLongitude, out grazeLattitude, out grazeAltitude,
                                 out solarLongitude, out solarLattitude);
        }

        public void GetGrazingPointInner(long time42, double[] ladeeState, double[] grazeVector,
                                         out double grazeLongitude, out double grazeLattitude, out double grazeAltitude,
                                         out double solarLongitude,
                                         out double solarLattitude)
        {
            var ladeeToMoon = new double[3];
            vminus_c(ladeeState, ladeeToMoon);
            var limb = new SpiceEllipse();
            var plane = new SpicePlane();
            var limbIntersectionCount = 0;
            var grz1 = new double[3];
            var grz2 = new double[3];

            // Calculate the graze limb point
            edlimb_c(MoonRadius, MoonRadius, MoonRadius, ladeeState, ref limb); // First, get the limb ellipse
            psv2pl_c(ladeeState, ladeeToMoon, grazeVector, ref plane);

            // Get a plane to cut it in the ram and anti ram directions
            inelpl_c(ref limb, ref plane, ref limbIntersectionCount, grz1, grz2); // grz1 = point in direction
            // Confirm there are two limb points
            if (limbIntersectionCount != 2)
                Console.WriteLine("Problem here");

            Normalize(grazeVector);

            var ladee = new double[3];
            ladee[0] = ladeeState[0];
            ladee[1] = ladeeState[1];
            ladee[2] = ladeeState[2];

            var vecGrz1 = new double[3];
            vecGrz1[0] = grz1[0] - ladee[0];
            vecGrz1[1] = grz1[1] - ladee[1];
            vecGrz1[2] = grz1[2] - ladee[2];
            Normalize(vecGrz1);

            var vecGrz2 = new double[3];
            vecGrz2[0] = grz2[0] - ladee[0];
            vecGrz2[1] = grz2[1] - ladee[1];
            vecGrz2[2] = grz2[2] - ladee[2];
            Normalize(vecGrz2);

            var ang1 = vdot_c(grazeVector, vecGrz1);
            var ang2 = vdot_c(grazeVector, vecGrz2);

            var pnt = ang1 > ang2 ? grz1 : grz2;

            grazeLongitude = grazeLattitude = grazeAltitude = 0d;

            recpgr_c("MOON", pnt, MoonRadius, 0d, ref grazeLongitude, ref grazeLattitude, ref grazeAltitude);
            //Console.WriteLine("   pnt: lon={0} lat={1} alt={2}", ToDegrees(grazeLongitude), ToDegrees(grazeLattitude), grazeAltitude);

            grazeLongitude = ToDegrees(grazeLongitude);
            grazeLattitude = ToDegrees(grazeLattitude);

            var et = TimeUtilities.Time42ToET(time42);
            var xform = GetMoonFixedToMoonSolarTransform(et);
            var solarGz = new double[3];
            mxv_c(xform, pnt, solarGz);

            // I don't understand the bug, but the solar lat is negative
            solarGz[2] = -solarGz[2];

            // convert to lon, and lat
            solarLongitude = solarLattitude = 0d;
            var altitude = 0d;
            recpgr_c("MOON", solarGz, MoonRadius, MoonFlattening, ref solarLongitude, ref solarLattitude, ref altitude);
            solarLattitude = ToDegrees(solarLattitude);
            solarLongitude = ToDegrees(solarLongitude);
        }
        ...
        public void GetWestGrazingPoint(long time42, out double grazeLongitude, out double grazeLattitude, out double grazeAltitude, out double solarLongitude, out double solarLattitude)
        {
            var ladeeState = new double[6];
            var et = TimeUtilities.Time42ToET(time42);
            var lt = 0d;
            ...
            GetGrazingPointInner(time42, ladeeState, grazeVector, out grazeLongitude, out grazeLattitude, out grazeAltitude, out solarLongitude, out solarLattitude);
        }

        public void GetNorthGrazingPoint(long time42, out double grazeLongitude, out double grazeLattitude, out double grazeAltitude, out double solarLongitude, out double solarLattitude)
        {
            var ladeeState = new double[6];
            var et = TimeUtilities.Time42ToET(time42);
            var lt = 0d;
            ...
            // The LADEE's state in ME frame
            spkgeo_c(-12, et, "MOON_ME", 301, ladeeState, ref lt);
            var grazeVector = ZAxis;
            GetGrazingPointInner(time42, ladeeState, grazeVector, out grazeLongitude, out grazeLattitude, out grazeAltitude, out solarLongitude, out solarLattitude);
        }

        public void GetSouthGrazingPoint(long time42, out double grazeLongitude, out double grazeLattitude, out double grazeAltitude,
                                         out double solarLongitude, out double solarLattitude)
        {
            var ladeeState = new double[6];
            var et = TimeUtilities.Time42ToET(time42);
