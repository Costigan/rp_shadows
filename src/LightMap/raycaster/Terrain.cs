...
namespace LightMap.raycaster
{
    public partial class Terrain
    {
        ...
        public float GridResolution = 0.4f;
        ...
        public void SetMinMax()
        {
            ...
            MinPX = 0f;
            MaxPX = (w - 1) * GridResolution;
            MinPY = 0f;
            MaxPY = (h - 1) * GridResolution;
            MinPZ = float.MaxValue;
            MaxPZ = float.MinValue;
            for (var y=0;y<h;y++)
                for (var x=0;x<w;x++)
                {
                    var v = HeightMap[x, y]; // Is this the right order
                    if (v < MinPZ) MinPZ = v;
                    if (v > MaxPZ) MaxPZ = v;
                }
        }
        ...
        public Vector3 Copy(Vector3 f) => new Vector3(f.X, f.Y, 0f);
            ...
            int sunRayHorizontalCount = 0)
        {
            ...
            var sampleStarts = Program.EstimateTime ? starts.Take(10).ToList() : starts;
            ...
            var sampleRayCount = sampleStarts.Count * raysCount;

            Console.WriteLine(@"Ray count for full image = {0}", totalRayCount);
            const float SecondsPerRay = 0.0302444f;
            var timespan = new TimeSpan(0, 0, (int)(SecondsPerRay * totalRayCount));
            Console.WriteLine(@"Estimated calculation time     : {0}", timespan);
            Console.WriteLine(@"Output file should be written at {0}", DateTime.Now.Add(timespan));

            var stopwatch = new System.Diagnostics.Stopwatch();
            stopwatch.Start();

            Enumerable.Range(0, sampleRayCount).AsParallel().ForAll(index =>
            {
                var startIndex = index / raysCount;
                var rayIndex = index - startIndex * raysCount;
                var s = sampleStarts[startIndex];
                ...
                for (var i = 1; i < max; i++)
                {
                    ...
                    //if (true)
                    {
                        ...
                        // This is part of the algorithm.  I'm turning it off for a quick test.  2017-03-25
            ...
            stopwatch.Stop();
            Console.WriteLine(@"Elapsed time={0}  time_per_ray={1} usec", stopwatch.Elapsed, (double)(1000L * stopwatch.ElapsedMilliseconds) / sampleRayCount);
        }
        ...
        public unsafe Bitmap ShadowBufferToScaledImageV4(Bitmap input = null)
        {
            var width = Width;
            var height = Height;
            Bitmap result = input;
