...
namespace LadeeViz.Viz
{
    public class OpenGLControlWrapper : GLControl
    {
        ...
        public Presentation PresentationMode = new PhasingLoopsOverview();
        ...
        public void DrawStars()
        {
            ...
            if (TheWorld.Stars != null)
                TheWorld.Stars.Draw(false, CameraMode.Eye);
        ...
        public void DrawTrajectory(Vector3d eye)
        {
            if (TheWorld.Trajectory == null) return;
            TheWorld.Trajectory.Draw(false, eye);
        }

        public void DrawLadeePoint(Vector3d eye)
        {
            GL.Disable(EnableCap.Lighting);
            GL.Color3(Color.LightBlue);
            GL.PointSize(8f);
            GL.Begin(BeginMode.Points);
            GL.Vertex3((TheWorld.LADEE.Position - eye).ToFar());
            GL.End();
            GL.Enable(EnableCap.Lighting);
        }
        ...
        // Testing for backface culling of DEMPatches
        // Not needed yet.
        private void TransformOrigin()
        {
            var o = new Vector3d();
            Vector3 o1 = (o - CameraMode.Eye).ToFar();
            var o2 = new Vector3(o1);
            Vector3 p = Vector3.Transform(o2, CameraMode.ViewMatrix);
            //Console.WriteLine(@"  transform {0}", CameraMode.ViewMatrix);
            Console.WriteLine(@"  Transform origin to [{0}, {1}, {2}]", p.X, p.Y, p.Z);
        }
